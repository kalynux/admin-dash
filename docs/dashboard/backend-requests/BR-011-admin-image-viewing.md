...
[`files.md`](../../admin/api/files.md) is explicit and good: *"the caller is already holding the id,
...
- [ ] A permission distinct from `files.resolve`, catalogued in
      [`permissions.md`](../../admin/api/permissions.md) and **not** marked `†`
...
- [ ] `FILE_VIEW_NOT_SUPPORTED` (or equivalent) is a distinct code in
      [`errors.md`](../../admin/api/errors.md) **and** in `error-codes.ts`, so a provider that
