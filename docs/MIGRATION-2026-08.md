...
| Suite | Parses |
|---|---|
| `src/types/permissions.types.test.ts` | `docs/admin/api/permissions.md` |
| `src/i18n/error-catalog.test.ts` | `docs/admin/api/errors.md` |
...
🔴 **Correction to the plan that produced this document.** It states the entire `dashboard/` folder
...
drift tool maps `docs/admin/` → `backend/admin/docs/`, and `docs/dashboard/` is a *sibling* of
`docs/admin/`, so it fell outside every configured pair and was reported as absent. Filed as
**F-56**.
