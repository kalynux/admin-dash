...
**Description**: Manually assign a plan to a vendor. This is an **admin override** for comps, support fixes, or migrations — the normal path is the vendor buying a plan themselves (`POST /vendor/plans/:planId/purchase`, see the vendor billing doc (not mirrored here — `backend/jovi-mall/api-doc/vendor/billing.md`)), which auto-activates on payment with no admin step. This endpoint applies the **same two-plan rule** without requiring a payment:
