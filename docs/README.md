...
| **see your own questions and their answers** | [`dashboard/`](dashboard/) |
...
├── dashboard/    ← your own backend-requests and the answers (14 files)
...
- **`dashboard/` lives one level up**, at `docs/dashboard/`, not `docs/admin/dashboard/`. Fourteen
  files under `src/` carry comments pointing at `docs/dashboard/…`, so moving it would break them.
- **`error-codes.ts`** mirrors a backend *source* file rather than a doc page. It has no upstream
  counterpart in `docs/` and will always report as an orphan.
...
belief that produced it. Filed as [BR-014](dashboard/backend-requests/BR-014-content-wire-shapes.md);
...
diff -r backend/admin/docs frontend/admin-dash/docs/admin

# your backend-requests — expect silence
diff -r backend/admin/docs/dashboard frontend/admin-dash/docs/dashboard
...
| Broken links in the **verbatim mirrors** (`admin/`, `dashboard/`) | **43** |
...
**The 43 in `admin/` and `dashboard/` are left alone on purpose**, because those two are byte-exact
mirrors and repairing them would end the one-command check above. They split into:

- **28 cross-repo** — into `backend/*/src/`, `PRODUCTION-READINESS/`, or a sibling service's doc
  folder. Unresolvable from a frontend repository, and **broken in the backend's own tree too**
  (21 of them, measured).
- **15 from the `dashboard/` placement.** Upstream, `dashboard/` sits *inside* `admin/docs/`; here
  it sits beside `admin/`, because 14 files under `src/` point at `docs/dashboard/…`. So
  `ADR-018`'s `./dashboard/…` links and `dashboard/`'s own `../api/…` links resolve against the
  backend's layout, not this one. **Read `./dashboard/x` as `../dashboard/x`, and `../api/x` as
  `../admin/api/x`.**
...
```bash
cp backend/admin/src/core/errors/error-codes.ts   frontend/admin-dash/docs/admin/error-codes.ts
cp backend/jovi-mall/src/core/error-codes.ts      frontend/admin-dash/docs/jovi-mall/error-codes.ts

cp backend/admin/src/modules/content/validators/article-body.validator.ts \
   frontend/admin-dash/docs/admin/article-blocks.ts
cp backend/admin/src/modules/content/domain/content.types.ts \
   frontend/admin-dash/docs/admin/content-domain.ts
cp backend/admin/src/modules/content/read-models/article.dto.ts \
   frontend/admin-dash/docs/admin/content-dto.ts
cp backend/admin/src/modules/content/validators/article.validator.ts \
   frontend/admin-dash/docs/admin/content-validators.ts

cp backend/jovi-mall/src/modules/tickets/types/ticket.types.ts \
   frontend/admin-dash/docs/jovi-mall/ticket-vocabularies.ts
cp backend/jovi-mall/src/modules/orders/order-timeline.model.ts \
   frontend/admin-dash/docs/jovi-mall/order-timeline-events.ts
```
