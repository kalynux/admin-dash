...
template-registry.ts (`jovi-mall/src/modules/whatsapp/handlers/template/template-registry.ts`)
...
- In-window sends use the same copy as **free-form text / interactive CTA** (no
  ...
  notification-catalog.ts (`jovi-mall/src/modules/notifications/catalog/notification-catalog.ts`).
- **Field limits:** Meta caps button labels at 20 chars, header/footer at 60,
  ...
  (whatsapp-limits.ts (`jovi-mall/src/modules/whatsapp/constants/whatsapp-limits.ts`)),
  but keep the template copy within them so nothing is truncated.

Languages are kept in sync with `SUPPORTED_LANGUAGES`
(core/constants/languages.ts (`jovi-mall/src/core/constants/languages.ts`)).
...
Sent by `DeliveryCodeService` (`jovi-mall/src/modules/cod/services/delivery-code.service.ts`)
...
notification-catalog.ts (`jovi-mall/src/modules/notifications/catalog/notification-catalog.ts`)
(vendor), agency-notification-catalog.ts (`jovi-mall/src/modules/notifications/catalog/agency-notification-catalog.ts`),
agent-notification-catalog.ts (`jovi-mall/src/modules/notifications/catalog/agent-notification-catalog.ts`).
...
source-of-truth in
agency-notification-catalog.ts (`jovi-mall/src/modules/notifications/catalog/agency-notification-catalog.ts`)** —
...
n8n/bot-surface.md § 13 (not mirrored here — `backend/jovi-mall/api-doc/n8n/bot-surface.md`).
...
customer-notification-catalog.ts (`jovi-mall/src/modules/notifications/catalog/customer-notification-catalog.ts`).
