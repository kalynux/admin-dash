...
```
`status` ∈ `pending | paid | failed`. `subscriber_plan_id` is the `SubscriberPlan` created once the purchase is applied (null until `paid`). (Owner-scoped: `owner_type`/`owner_id` replace the old `vendor_id`; the same rows serve agency/agent purchases.)
