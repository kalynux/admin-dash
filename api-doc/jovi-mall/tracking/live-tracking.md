...
> `GET /api/agency/tracking/board` (not mirrored here — `backend/jovi-mall/api-doc/agency/live-tracking.md`). That is the
...
client renews via
`POST /api/auth/mobile/refresh` (not mirrored here — `backend/jovi-mall/api-doc/auth/FRONTEND-CHANGELOG-mobile-auth.md`). Either
