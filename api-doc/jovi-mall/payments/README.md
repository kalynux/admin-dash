...
| Flow | Where |
|---|---|
| Booking payment + its own status poll | `POST /api/bookings/:id/pay`, `GET /api/bookings/:id/payment-status` — ../customer/bookings.md (not mirrored here — `backend/jovi-mall/api-doc/customer/bookings.md`) |
| Gateway webhooks (server-to-server) | `POST /api/webhooks/*` — not client-callable |
| **Plan purchases & credit top-ups** | `/{vendor,agency,agent}/plans/...`, `.../credits/topups` — a **separate** path that creates **no** `PaymentTransaction`. See ../billing-plans-across-roles.md (not mirrored here — `backend/jovi-mall/api-doc/billing-plans-across-roles.md`) |
| Saved cards / mobile-money instruments | `/api/me/payment-methods` — ../customer/payment-methods.md (not mirrored here — `backend/jovi-mall/api-doc/customer/payment-methods.md`) |
...
- **COD orders never touch this surface.** Cash on delivery is settled by the agent submitting the
  customer's delivery code; there is no gateway call. See ../agent/cod-cash.md (not mirrored here — `backend/jovi-mall/api-doc/agent/cod-cash.md`)
  and ../customer/orders.md (not mirrored here — `backend/jovi-mall/api-doc/customer/orders.md`).
...
- ../customer/orders.md (not mirrored here — `backend/jovi-mall/api-doc/customer/orders.md`) — checkout, cart groups, and where `initiate` fits
- ../customer/bookings.md (not mirrored here — `backend/jovi-mall/api-doc/customer/bookings.md`) — booking payment and its own status endpoint
- ../customer/payment-methods.md (not mirrored here — `backend/jovi-mall/api-doc/customer/payment-methods.md`) — saved instruments
- ../billing-plans-across-roles.md (not mirrored here — `backend/jovi-mall/api-doc/billing-plans-across-roles.md`) — plans/credit, a separate payment path
