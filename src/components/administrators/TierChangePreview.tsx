            ...
            <CollapsibleContent>
                <ul className="text-muted-foreground max-h-48 space-y-0.5 overflow-y-auto px-3 py-2 font-mono text-[11px]">
                    {/*
                      ...
                      Mono but not copyable: this is a vocabulary dump — up to
                      114 names in one scroller — and the sweep's rules keep
                      enum values out. A copy button per row would put a hundred
                      identical controls in a list nobody pastes out of.
                    */}
                    {names.map((name) => (
                        <li key={name}>{name}</li>
                    ))}
