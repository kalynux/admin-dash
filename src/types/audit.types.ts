 ...
 *
 * ── Deliberately not an `AuditEntry` ──────────────────────────────────────────
 * A legacy row has no catalogued action, no `subjectClass` and no `sensitive`
 * flag, because jovi-mall's middleware knows none of this service's vocabulary.
 * Forcing it into the real shape would mean lying about those fields or filling
 * them with nulls a reader then has to special-case — and either way the
 * compliance record and the stop-gap stop being distinguishable at a glance.
 *
 * **The whole module is deleted at cutover**, when `meta.unportedEndpoints`
 * reaches zero.
 */
export interface LegacyAuditEntry {
    ...
    occurredAt: string;
    correlationId: string;
    /** Always `'jovi-mall-legacy'`. On every item, not just in `meta`. */
    source: string;
    /**
     * `request` — the coarse per-request row, on which `action`, `resource` and
     * `changes` are all `null`, because the middleware records what was called
     * rather than what it meant. `service` — a row naming a specific action.
     */
    kind: 'request' | 'service' | (string & {});
    actor: LegacyAuditActor;
    request: {
        method: string;
        path: string;
        statusCode: number | null;
...
/**
 * ⚠ **`platform_admin` is *not* a wi-admin administrator.**
 *
 * Two identity spaces: a wi-admin administrator holds no jovi-mall `users` row
 * at all, and no mapping between them exists. `label` is rendered server-side
 * precisely so a client cannot accidentally present the row as one of ours —
 * render it as given rather than composing a name from the other fields.
 */
export interface LegacyAuditActor {
    kind: 'platform_admin' | 'platform_user' | 'anonymous' | (string & {});
    label: string;
    userId: string | null;
    role: string | null;
    name: string | null;
...
/**
 * `GET /audit/legacy` query — narrower than the real feed's on purpose.
 *
 * **There is no `search` and no `sort`.** Every field worth filtering on is an
 * exact match, and not offering a text search removes a query-composition trap
 * rather than defusing it.
 */
export interface LegacyAuditListQuery {
    /** A jovi-mall `users` id — exactly 24 characters. */
    actorUserId?: string;
    /** **Free text, 1–100 chars, not an enum** — jovi-mall's verbs are its own vocabulary. */
    action?: string;
    resourceType?: string;
    source?: 'request' | 'service' | (string & {});
    /** Coerced leniently here, unlike the strict instants elsewhere. */
    from?: string;
    to?: string;
    page?: number;
    limit?: number;
 ...
 * compliance record by omission. Surface all four.
 */
export interface LegacyAuditListMeta {
    total: number;
    page: number;
    limit: number;
    /** `0` on an empty list, not `1`. */
    ...
    /** Always `true`. */
    retiresAtCutover: boolean;
    /** How much legacy surface remains. **At 0 this feed is deleted.** */
    unportedEndpoints: number | null;
}
